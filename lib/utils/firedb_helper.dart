...
import 'package:final_advance_exam/screen/home/model/firestore_model.dart';
...
class CloudFirestoreHelper {
  ...
  Future<void> addData(FireStoreModel model) async {
    await db
        .collection("shopping")
        .add({"name": model.name, "price": model.price, "dec": model.dec});
  }
  ...
  Future<void> updateData(FireStoreModel model) async {
    await db
        .collection("shopping")
        .doc(model.id)
        .set({"name": model.name, "price": model.price, "dec": model.dec});
  }
